...
(async () => {
  ...
  document.body.insertAdjacentHTML('beforeend', `<p>TEST1: SO Classic worker</p>`);
  try {
...
(async () => {
  ...
  document.body.insertAdjacentHTML('beforeend', `<p>TEST3: SO Classic worker SO importScript</p>`);

  try {
    const worker = new Worker('https://localhost:8443/worker_classic_importer_so.js');
    worker.onmessage = (e) => {
    ...
    console.log('TEST3: posted to worker: Hello SO classic worker SO importScript');
  } catch (e) {
    console.error('TEST3 FAIL: SO classic worker importScript SO failed to load:', e);
    document.body.insertAdjacentHTML(
      'beforeend',
...
(async () => {
  ...
  document.body.insertAdjacentHTML('beforeend', `<p>TEST4: SO Classic worker CO importScript</p>`);
  try {
    const worker = new Worker('https://localhost:8443/worker_classic_importer_co.js');
    worker.onmessage = (e) => {
